...
const createProperty = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const photoUrls = yield Promise.all(files.map((file) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            // Perform the upload
            yield new lib_storage_1.Upload({ client: s3Client, params: uploadParams }).done();
            // Verify uploaded image
            const getObject = new client_s3_2.GetObjectCommand({ Bucket: process.env.S3_BUCKET_NAME, Key });
            const s3Object = yield s3Client.send(getObject);
            const buffer = yield new Promise((resolve, reject) => {
                streamToBuffer(s3Object.Body, (err, buf) => {
                    if (err)
                        reject(err);
                    else
                        resolve(buf);
            ...
            const isValidImage = buffer.slice(0, 4).equals(Buffer.from([0xff, 0xd8, 0xff])) || // JPEG
